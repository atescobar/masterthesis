...
c = np.ones(M)
positions = [-1, 0, 1]
A = sp.sparse.spdiags(np.array([a, b, c]), positions, M, M).todense()
...
for n in range(0,N-1):
    for k in range(1, M-1):
        ...
        #Cp[n+1,k] = Cp[n,k] * (1 - rp * (-2 + Psi[n,k] - Psi[n,k-1])) - rp * Cp[n,k+1] * (1 + Psi[n,k] - Psi[n,k+1]) - rp * Cp[n,k-1]
        Cp[n + 1, k] = rp * Cp[n, k + 1] * (1 + Psi[n, k] - Psi[n, k + 1]) + Cp[n, k] * (1 + rp * (-2 + Psi[n, k] - Psi[n, k - 1])) + rp * Cp[n, k - 1]
        #Cm[n+1,k] = Cm[n,k] * (1 - rm * (-2 - Psi[n,k] + Psi[n,k-1])) - rm * Cm[n,k+1] * (1 + Psi[n,k+1] - Psi[n,k]) - rm * Cm[n,k-1]
        Cm[n + 1, k] = rm * Cm[n, k + 1] * (1 + Psi[n, k + 1] - Psi[n, k]) + Cm[n, k] * (1 + rm * (-2 + Psi[n, k - 1] - Psi[n, k])) + rm * Cm[n, k - 1]
        if (k == M - 1):
            Cp[n + 1, k] = Cp[n + 1, k] - dt * r
        if(k == 1 and n > 0):
            b[n+1,k] = -k ** 2 * (Cp[n+1,k] - Cm[n+1, k])-Psi0
        else:
            b[n+1,k] = -k ** 2 * (Cp[n+1,k] - Cm[n+1, k])

    print("\rCompleted: " + "time axis: " +str("{0:.2f}".format((n / N) * 100)) + " %" + "\t" + "X axis: " + str("{0:.2f}".format((k / M) * 100)) +" %" , end="")
